...
function showUser(){
  ...
  function helloUser(){
    let checkLogin = JSON.parse(localStorage.getItem("boolLogin"))
    console.log(checkLogin);
    if ( checkLogin == true){
    let information = JSON.parse(localStorage.getItem("information"));
    let indexLogin = localStorage.getItem("indexLogin");
    console.log(information);
    console.log(indexLogin);
    console.log(information[indexLogin].username);
    document.getElementsByClassName('show-user')[0].textContent = information[indexLogin].username;
